...
public record HashAggregateExec(
    ...
    public Sequence<RecordBatch> execute() {
        ...
        var ss = StreamSupport.stream(Spliterators.spliteratorUnknownSize(input.execute().iterator(), Spliterator.ORDERED), false);
        ss.forEach(batch -> {
            var groupKeys = groupExpr.stream().map(it -> it.evaluate(batch)).toList();
            var aggInputValues = aggregateExpr.stream().map(it -> it.inputExpression().evaluate(batch)).toList();
            ...
            for (var rowIndex = 0; rowIndex < batch.rowCount(); rowIndex += 1) {
                ...
                if (accumulators == null) {
                    accumulators = aggregateExpr.stream().map(AggregateExpression::createAccumulator).toList();
                }
        ...
        IntStream.range(0, map.size()).forEach(rowIndex -> {
            ...
            IntStream.range(0, groupExpr.size()).forEach(it -> builders.get(it).set(rowIndex, groupingKey.get(it)));
            IntStream.range(0, aggregateExpr.size()).forEach(it -> builders.get(groupExpr.size() + it).set(rowIndex, accumulators.get(it).finalValue()));
        });
